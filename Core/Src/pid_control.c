             ...
             float actual_value)
{
...
#ifdef LIMIT_CHECKING
        pid_value =
            checkLimits(pid_value, p_pid_handler->max_p, p_pid_handler->min_p);
#endif
        output += pid_value;
    }
...
#ifdef LIMIT_CHECKING
        pid_value =
            checkLimits(pid_value, p_pid_handler->max_d, p_pid_handler->min_d);
#endif
        output += pid_value;
    }
    ...
    if (p_pid_handler->ki != 0)
    {
...
#ifdef LIMIT_CHECKING
        pid_value =
            checkLimits(pid_value, p_pid_handler->max_i, p_pid_handler->min_i);
#endif
        output += pid_value;
        ...
        if (output > p_pid_handler->max_out)
        {
            ...
            /* since output is saturated at max value, prevent integral from
             * increasing.*/
            if (pid_value > 0)
            {
                pid_value = 0;
        ...
        if (output < p_pid_handler->min_out)
        {
            ...
            /* since output is saturated at min value, prevent integral from
             * decreasing.*/
            if (pid_value < 0)
            {
                pid_value = 0;
